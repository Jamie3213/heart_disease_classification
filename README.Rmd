...
* Patients in the sample population are most likely to be in their late 50s, though we can also see a smaller peak in the density curve in the mid-40s, as well;
...
* The distributions for both cholesterol ```chol``` and maximum heart rate ```thalach``` look to be fairly normal (though there's a slight bumb in the heart rate curve), with peaks around ```240``` and ```160```, respectively;
...
Now that we've looked at both the continuous and categorical variables in the, we have a better understanding of the data and we can move on to modelling.

## Training a model

We're going to start the modelling process by writing a function to train a model using a given algorithm - the function will:
...
We can see from the above results that the multiple logistic regression model has an accuracy of roughly 83% which is impressive for such a simple model! The model has also returned a ```Kappa``` value of roughly ```0.66``` - this is Cohen's kappa which essentially gives a measure of the model accuracy taking into account the likelihood that the model could've classified some observations correctly by chance. Let's see which of the features were most influential by looking at the variable importance scores:
